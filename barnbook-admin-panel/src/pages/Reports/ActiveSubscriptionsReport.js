...
export default function ActiveSubscriptionsReport() {
  ...
  const getData = async () => {
    try {
      ...
      const responseData = await fetchData("api/masters/getActiveSubscriptionReport", payload);
  ...
  const generateColumns = (data, columnsToShow) => {
    ...
    const columns = columnsToShow.map((col) => {
      ...
      return {
        ...
        Cell: ({ value }) => {
          // Format date values
          if (["start_date", "actual_end_date"].includes(col) && value) {
            const date = new Date(value);
