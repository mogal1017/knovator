...
export default function ChurnRateReport() {
  ...
  const customColumnNames = {
    ...
    "subscriptions.discounted_amount": "Amount Paid",
  ...
  const getData = async () => {
    ...
    try {
      ...
      const responseData = await fetchData("api/masters/churnRateReport", payload);
  ...
  const generateColumns = (data, columnsToShow) => {
    ...
    const columns = columnsToShow.map((col) => {
      ...
      return {
        ...
        Cell: ({ value }) => {
          // Format date values
          if (["start_date", "end_date", "actual_end_date"].includes(col) && value) {
            const date = new Date(value);
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          ...
          >
            <Breadcrumbs title="Churn Rate Report" breadcrumbItems={breadcrumbItems} />
